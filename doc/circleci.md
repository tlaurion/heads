...
### Cache (REPLACES workspace in current model)

A CircleCI cache is stored for reuse by later pipeline runs in the same
repository.

- Caches help future pipelines (inter-pipeline reuse)
- Caches do NOT help sibling jobs in the same workflow run
...
- No workspace persistence - cache saves always run after build succeeds
- This eliminates the workspace failure point that blocked cache save

### Cache keys (format: {arch}-{layer}-nix-docker-heads-{hash}[-{fork}])
- `{arch}-musl-cross-make-nix-docker-heads-{hash}` - Base toolchain
- `{arch}-coreboot-musl-cross-make-nix-docker-heads-{hash}-{fork}` - Coreboot + musl
- `{arch}-modules-coreboot-musl-cross-make-nix-docker-heads-{hash}-{fork}` - Full build

---

## x86 pipeline shape (cache-only, no workspace)

The x86 chain runs jobs in parallel where possible:

1. `create_hashes` (hash generation)
2. `x86_blobs` (blobs download, cached)
3. `x86_musl_cross_make` (musl toolchain, cached)
4. `x86_coreboot` seed jobs, one per coreboot fork (each saves modules + coreboot cache)
5. Downstream board builds for each fork, in parallel (restore cache directly)

All jobs use cache for state transfer - no workspace persistence.

For the coreboot 25.09 branch, the seed board is `EOL_t480-hotp-maximized`.
That job saves the modules and coreboot caches used by other 25.09 boards.
...
- Each coreboot fork now saves both modules and coreboot caches, eliminating the single-point-of-failure.
...
- For x86 coreboot forks, each fork saves its own modules cache keyed by `{coreboot_dir}`.
