...
**MSI Z690/Z790:** These desktop boards require a CPU with integrated
Intel graphics (no F-suffix CPU) because the motherboard has no onboard GPU
chip.  The iGPU is initialized by FSP GOP and the display runs via the
fbdev console.
when DRM is not compiled — the other 8 x86 boards with `# CONFIG_DRM is not
set` carry it as dead config.
...
"vesa-framebrowser" (based on orig_video_isVGA=0x23 from our patch 0003),
vesadrm binds.  Stride validation passes because __screen_info_lfb_size()
...
|----------|-----------|----------------|
| simledrm_sysfb | Ubuntu 26.04 Live on nv4x_adl | `dmesg \| grep simple-framebrowser` — should show simpledrm bind |
| vesadrm | openSUSE Tumbleweed on nv4x_adl | `dmesg \| grep vesa-framebrowser` — should show vesadrm bind, stride OK |
...
cannot bind because no "simple-framebrowser" device is created.
...
## What we changed (patches 0001-0003)

Heads' kexec-tools patches modify `setup_linux_vesafb()` in
`kexec/arch/i386/x86-linux-setup.c` to normalize screen_info before
passing it to the target kernel:

| Change | Patch | Effect |
|--------|-------|--------|
| `orig_video_isVGA = 0x23` (VLFB) | 0003 | sysfb creates "vesa-framebrowser".  Avoids 7.x lfb_size stride validation bug. |
...
## Current kernel symbol detection (functions.sh lines 3315-3467)

`_check_kernel_probe_driver()` searches the decompressed kernel for:

| Symbol | Searched for | Driver | Binds with VLFB? | Assessment |
|--------|-------------|--------|-----------------|------------|
| `vesadrm_probe` | ✅ | vesadrm (DRM, 7.x) | ✅ YES — to "vesa-framebrowser" | **KEEP** — verified on openSUSE 7.x |
| `vesadrm_platform_driver_init` | ✅ | (same) | ✅ | **KEEP** |
| `simpledrm_probe` | ✅ | simpledrm (DRM) | ✅ via SYSFB_SIMPLEFB=y | **KEEP** |
| `simpledrm_driver_init` | ✅ | (same) | ✅ | **KEEP** |
| `sysfb_parse_mode` (add'l) | ✅ | SYSFB_SIMPLEFB | ✅ confirms simple-fb device created | **KEEP** |
| `vesafb_probe` | ✅ | vesafb (fbdev) | ✅ on 5.x/6.x, binds to "vesa-framebrowser" | **KEEP** |
...
## ISO analysis: initramfs frameworks discovered

Seven frameworks found across all ISOs:

| Framework | Distros | ISO param | Needs from Heads |
|-----------|---------|-----------|------------------|
| casper | Ubuntu, PureOS | `iso-scan/filename=` | ✅ `iso-scan/filename=/$iso_path` |
| live-boot | Debian, Tails, Kicksecure | `boot=live` + `findiso=` | ✅ `findiso=/$iso_path` (boot=live comes from ISO) |
| dmsquash-live | Fedora, Qubes (via Xen) | `root=live:` + `iso-scan/filename=` | ⚠️ `root=live:` only from ISO's own cfg |
| kiwi-live | openSUSE | `root=live:` + `iso-scan/filename=` | ⚠️ Same |
| stage-1 | NixOS | `findiso=` | ✅ `findiso=/$iso_path` |
| EFI firmware | Samsung SSD | N/A | ❌ Requires UEFI runtime — cannot boot |
| Xen multiboot2 | Qubes | `inst.repo=hd:` | ❌ Not kexec-compatible |

All detection items above are implemented and verified (137/138 ISOs pass).
