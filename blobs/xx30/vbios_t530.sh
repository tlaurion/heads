...
#Add local path to call executables within this script
PATH="$PATH:$HOME/.nix-profile/bin"

echo "### Downloading rom-parser dependency"
wget -q https://github.com/awilliam/rom-parser/archive/"$ROMPARSER".zip || { echo "Failed to download $ROMPARSER" && exit 1; }
...
unzip "$ROMPARSER".zip
cd rom-parser-"$ROMPARSER" && make > /dev/null 2>&1 
#Add local path to call executables within this script
PATH+="./"


echo "### Downloading UEFIExtract dependency"
wget -q https://github.com/LongSoft/UEFITool/releases/download/A58/"$UEFIEXTRACT" || { echo "Failed to download $UEFIEXTRACT" && exit 1; }
...
unzip "$UEFIEXTRACT"  > /dev/null 2>&1
#Add local path to call executables within this script
PATH+="./"
echo "local dir: $(pwd)"

echo "### Downloading VBiosFinder"
wget -q https://github.com/coderobe/VBiosFinder/archive/"$VBIOSFINDER".zip || { echo "Failed to download $VBIOSFINDER" && exit 1; }
...
unzip "$VBIOSFINDER".zip  > /dev/null 2>&1
...
wget -q https://download.lenovo.com/pccbbs/mobiles/"$BIOSUPDATE" || { echo "Failed to download $BIOSUPDATE" && exit 1; }
...
innoextract "$extractdir"/rom-parser-"$ROMPARSER"/VBiosFinder-"$VBIOSFINDER"/"$BIOSUPDATE"  > /dev/null 2>&1 || { echo "Failed to extract $BIOSUPDATE" && exit 1; }

echo "### Finding, extracting and saving vbios"
eval ./vbiosfinder extract "$extractdir"/rom-parser-"$ROMPARSER"/VBiosFinder-"$VBIOSFINDER"/"app/G4ETB7WW/\$01D5100.FL1" || { echo "Failed to extract FL1" && exit 1; }
