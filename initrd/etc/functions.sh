...
_check_kernel_probe_driver() {
	...
	for ((pos = 0; pos <= ${#probe_hex} - 12; pos += 2)); do
		...
		if [ "$dsize" -gt 0 ] 2>/dev/null; then
			...
			#
			# Priority 1: vesadrm (DRM sysfb, kernel 7.x SUSE path)
			#   Binds to "vesa-framebrowser" created by sysfb from VLFB.
			#
			# Priority 2: vesafb (fbdev, kernel 5.x/6.x)
			#   Binds to "vesa-framebrowser" on older kernels with FB_VESA.
			#
			...
			if [ "$driver" = "vesafb" ]; then
				...
				if [ -n "$_found_symbol" ]; then
					if echo "$_found_symbol" | grep -q 'vesadrm'; then
						...
						# create a "simple-framebrowser" device.  Verify the
...
#   1. PCI display controller (class 0x030000/0x030100) with bound driver
#   2. Active framebuffer device (/sys/class/graphics/fb0/name)
detect_active_gpu_driver() {
	TRACE_FUNC
	local dev class driver

	# Method 1: check PCI display controllers for a bound driver.
	# This tells us what native GPU driver is in use.
	for dev in /sys/bus/pci/devices/*; do
		...
		if [ -L "$dev/driver" ]; then
			driver=$(basename "$(readlink "$dev/driver")" 2>/dev/null || true)
			if [ -n "$driver" ]; then
				DEBUG "detect_active_gpu_driver: Method 1 (PCI) found driver=$driver"
				echo "$driver"
				return 0
			fi
		else
			local _pci_vendor="" _pci_device=""
			_pci_vendor=$(cat "$dev/vendor" 2>/dev/null | tr -d '\n' || true)
			_pci_device=$(cat "$dev/device" 2>/dev/null | tr -d '\n' || true)
			DEBUG "detect_active_gpu_driver: Method 1: PCI display device ${_pci_vendor}:${_pci_device} has no bound driver"
		fi
	done

	# Method 2: active framebuffer device name.
	# Works when no PCI driver is bound (fbdev console fallback).
	# NOTE: fb0 names (e.g. "bochs-drmdrmfb", "simpledrmdrmfb") are the
	#       framebuffer driver's label, NOT the kernel GPU module name.
	if [ -r /sys/class/graphics/fb0/name ]; then
		driver=$(cat /sys/class/graphics/fb0/name 2>/dev/null | tr -d '\n' || true)
		if [ -n "$driver" ]; then
			DEBUG "detect_active_gpu_driver: Method 2 (fb0) name=$driver (framebuffer label, not kernel module)"
			DEBUG "detect_active_gpu_driver: fb0 name '$driver' (fbdev console fallback)"
			echo "$driver"
			return 0
		fi
	fi

	DEBUG "detect_active_gpu_driver: no GPU driver detected"
	return 1
}
...
# Args: iso_params  add  remove  [board_add]
#   iso_params    --  ISO's original kernel cmdline
#   add           --  ADD params (universal + loopback.cfg) injected before ISO's
#   remove        --  params to remove (CONFIG_BOOT_KERNEL_REMOVE)
#   board_add     --  board-level ADD overrides (CONFIG_BOOT_KERNEL_ADD)
# Returns: final cmdline with:
#   1. REMOVE applied to both iso_params and add
#   2. ADD prepended, board_add appended
#   3. All 6 ISO-finding keys (iso-scan/filename, findiso, img_dev,
#      img_loop, iso, live-media) enforced  --  replace any ISO defaults
# Priority: board_add > injected keys > ISO originals
_build_final_cmdline() {
	local _iso_params="$1" _add="$2" _remove="$3" _board_add="$4"
	local _clean_add="" _combined=""

	# Clean ADD: strip GRUB --- separator, apply board REMOVE
	_clean_add=$(echo "$_add" | sed 's/ --- / /g;s/^--- //g;s/ ---$//g' | xargs)
	# BusyBox sed lacks \b word boundaries; use bash space-wrapped
	# substitution to remove whole-word params.
	for _rm_item in $_remove; do
		_clean_add=" $_clean_add "
		_clean_add="${_clean_add// $_rm_item / }"
		_clean_add="${_clean_add# }"
		_clean_add="${_clean_add% }"
	done
	_clean_add=$(echo "$_clean_add" | xargs)

	# Apply board REMOVE to ISO params
	for _rm_item in $_remove; do
		_iso_params=" $_iso_params "
		_iso_params="${_iso_params// $_rm_item / }"
		_iso_params="${_iso_params# }"
		_iso_params="${_iso_params% }"
	done
	...
	# Enforce ISO-finding keys: for each known key that we inject,
	# replace ALL occurrences in _combined with our value, then
	# deduplicate  --  remove every occurrence and re-append once.
	# This prevents duplicate keys when the ISO also defines them
	# (e.g. our live-media=... prepended, ISO's live-media=removable
	# replaced => two identical entries; dedup reduces to one).
	# Keys NOT injected by us are left untouched.
	# GREP uses (^| ) anchor to avoid matching inside other tokens
	# (e.g. "iso=" matches inside "findiso=").  SED replacement
	# uses a space anchor for the same reason (BusyBox sed lacks
	# extended regex grouping).
	for _key in iso-scan/filename findiso img_dev img_loop iso live-media; do
		_val=$(echo "$_clean_add" | grep -oE "(^| )$_key=[^ ]*" | head -1 | sed 's/^ //')
		if [ -n "$_val" ]; then
			# Replace all occurrences with our value
			_combined=$(echo "$_combined" | sed "s| $_key=[^ ]*| $_val|g")
			_combined=$(echo "$_combined" | sed "s|^$_key=[^ ]*|$_val|")
			# Remove all (now duplicate) copies, re-append once.
			# Safety counter prevents infinite loop if future keys
			# contain BRE metacharacters that break the sed match.
			_dedup_guard=0
			while echo "$_combined" | grep -q " $_key="; do
				_combined=$(echo "$_combined" | sed "s| $_key=[^ ]*||")
				_dedup_guard=$((_dedup_guard + 1))
				[ "$_dedup_guard" -gt 100 ] && break
			done
			_combined=$(echo "$_combined" | sed "s|^$_key=[^ ]*||")
			_combined="$_combined $_val"
		fi
	done

	# Append Board ADD last (always wins  --  never touched by enforce)
	echo "$_combined $_board_add" | xargs
}

# Map a GPU driver name to the kernel symbol prefix used for built-in
# detection.  Kernel modules use hyphens in filenames but underscores
# in generated symbols (module_pci_driver maco creates <name>_driver_init,
# <name>_pci_probe, etc.).  For example:
#   PCI driver "bochs-drm" -> symbols "bochs_drm_pci_probe"
#   PCI driver "ast"       -> symbols "ast_pci_probe"
_gpu_driver_to_kernel_symbol() {
	echo "$1" | tr '-' '_'
}
