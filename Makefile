...
define define_module =
  ...
  ifneq ("$($1_repo)","")
    ...
    #
    # XXX: "rm -rf" and "make -C" are a hack for coreboot during commit
    #      switching, need module-specific cleanup action to get rid of it.
	...
	if [ ! -e "$$@" ]; then \
		...
		rm -rf "$(build)/$($1_base_dir)/payloads/external/skiboot/build" && \
		rm -rf "$(build)/$($1_base_dir)/payloads/external/skiboot/skiboot" && \
		if [ -d "$(build)/$($1_base_dir)/util/cbmem" ]; then make -C "$(build)/$($1_base_dir)/util/cbmem" clean; fi && \
