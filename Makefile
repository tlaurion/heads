...
define define_module =
  ...
  ifneq ("$($1_repo)","")
	...
	if [ ! -e "$$@" ]; then \
		...
		echo "git -C "$(build)/$($1_base_dir)" clean -df" && \
		git -C "$(build)/$($1_base_dir)" clean -df && \
		echo "git -C "$(build)/$($1_base_dir)" clean -dffx payloads util/cbmem" && \
		git -C "$(build)/$($1_base_dir)" clean -dffx payloads util/cbmem && \
		echo "git -C "$(build)/$($1_base_dir)" fetch $($1_repo) $($1_commit_hash) --recurse-submodules=no" && \
		git -C "$(build)/$($1_base_dir)" fetch $($1_repo) $($1_commit_hash) --recurse-submodules=no && \
		echo "git -C "$(build)/$($1_base_dir)" reset --hard $($1_commit_hash)" && \
		git -C "$(build)/$($1_base_dir)" reset --hard $($1_commit_hash) && \
		echo "git -C "$(build)/$($1_base_dir)" submodule sync" && \
		git -C "$(build)/$($1_base_dir)" submodule sync && \
		echo "git -C "$(build)/$($1_base_dir)" submodule update --init --checkout" && \
